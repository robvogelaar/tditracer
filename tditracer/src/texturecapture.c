 ...
 */
    ...
    for (texture = 0; texture < nr_textures; texture++) {
        ...
        if (NULL == texture_ptr) {
            ...
            char fname[64];
            sprintf(fname, "t%u-%05d-f%03d-%s-%d-%dx%d+%d+%d.png",
                    texture_ptr->name, texture_ptr->id, texture_ptr->frame,
                    texture_ptr->ttype == PARENT
                        ? "p"
                        : texture_ptr->ttype == SUB ? "s" : "r",
                    FORMATSIZE(texture_ptr->format), texture_ptr->width,
                    texture_ptr->height, texture_ptr->xoffset,
                    texture_ptr->yoffset);
