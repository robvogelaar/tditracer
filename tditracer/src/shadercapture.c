...
void shadercapture_capshader(unsigned int name, int count, const char **string,
    ...
    shaderlinkedlist_add_to_list(current_shader, name, program, count,
                                 string_out, length_out, true);
...
void shadercapture_writeshaders(void) {
    ...
    for (shader = 0; shader < current_shader; shader++) {
        ...
        if (NULL == shader_ptr) {
            ...
            char fname[64];
            sprintf(fname, "s%02d-%08d-%08d.txt", shader_ptr->id,
                    shader_ptr->name, shader_ptr->program);
