...
#define CHAR_ARRAY_TO_U32(array)                                          \
  ((unsigned int)((array[0] << 24) | (array[1] << 16) | (array[2] << 8) | \
                  array[3]))
...
#define timeDoctor_task_create(Tid, Name)                  \
  timeDoctor_Info(TDI_COMMAND(TDI_CREATE, TDI_TASK) | Tid, \
...
#define timeDoctor_task_switch(NewTid, NewName, OldTid, OldName)            \
  timeDoctor_Info(TDI_COMMAND(TDI_STOP, TDI_TASK) | OldTid,                 \
                  CHAR_ARRAY_TO_U32(OldName), CHAR2_ARRAY_TO_U32(OldName)); \
  timeDoctor_Info(TDI_COMMAND(TDI_START, TDI_TASK) | NewTid,                \
                  CHAR_ARRAY_TO_U32(NewName), CHAR2_ARRAY_TO_U32(NewName))
