 ...
 *
 *	Initializes TI/Intel sk_buff data members. Usually used on just
 *	allocated buffers. currently from __alloc_skb and build_skb
 */
void skb_init_intel(struct sk_buff *skb)
{
	...
	memset((void *)&skb->pp_packet_info, 0, sizeof(skb->pp_packet_info));
	skb->pp_packet_info.egress_queue = TI_PPM_EGRESS_QUEUE_INVALID;
#ifndef CONFIG_MACH_PUMA5
    skb->pp_packet_info.pp_session.session_handle = AVALANCHE_PP_MAX_ACCELERATED_SESSIONS; /* Set invalid session */
#endif
#ifdef PPP_DEBUG
#ifdef CONFIG_FTRACE
	skb->pp_packet_info.ppp_packet_info.allocTime = sched_clock();
#endif
#endif
    ...
    skb->pp_packet_info.pp_session.cpe_number = AVALANCHE_PP_L2VPN_INVALID_CPE_NUMBER; // Indicating N.I.U cpe number
#endif
#endif
}
...
#ifdef CONFIG_TI_PACKET_PROCESSOR
    memcpy((void *)&n->pp_packet_info, (void *)&skb->pp_packet_info, sizeof(skb->pp_packet_info));
#ifdef PPP_DEBUG
    n->pp_packet_info.ppp_packet_info.num_clones++;
#ifdef CONFIG_FTRACE
    if (n->fclone == SKB_FCLONE_UNAVAILABLE)
        n->pp_packet_info.ppp_packet_info.allocTime = sched_clock();
#endif
#endif
#endif  /* CONFIG_TI_PACKET_PROCESSOR */
...
#ifdef CONFIG_TI_PACKET_PROCESSOR
    memcpy((void *)&new->pp_packet_info, (void *)&old->pp_packet_info, sizeof(old->pp_packet_info));
#ifdef PPP_DEBUG
    new->pp_packet_info.ppp_packet_info.num_copies++;
#endif
#endif  /* CONFIG_TI_PACKET_PROCESSOR */
}
